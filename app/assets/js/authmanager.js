...
exports.removeMojangAccount = async function(uuid){
    try {
        ...
        // --- XEONY MOD: Si es cuenta Offline, la borramos directamente sin llamar a la API ---
        if(authAcc.type === 'offline' || (authAcc.meta && authAcc.meta.type === 'offline')) {
...
exports.validateSelected = async function(){
    ...
    // --- XEONY MOD: Bypass Validación para cuentas Offline ---
    // Si es offline, siempre regresamos true para que no intente validar con servidores remotos.
    if(current.type === 'offline' || (current.meta && current.meta.type === 'offline')) {
        log.info('Validating offline account: Skipping remote checks.')
        return true
    }
    // ---------------------------------------------------------
